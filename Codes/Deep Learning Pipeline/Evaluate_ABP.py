...
def evaluate_BP_Classification_ABP(Ytrue, Ypred):
    ...
    DBP_Conf = {'y_Actual': cls_gt, 'y_Predicted': cls_pred}
    DBP_Conf = pd.DataFrame(data=DBP_Conf)
    ...
    SBP_Conf = {'y_Actual': cls_gt, 'y_Predicted': cls_pred}
    SBP_Conf = pd.DataFrame(data=SBP_Conf)
    print('SBPS Classification Accuracy')
    print(classification_report(cls_gt, cls_pred, digits=5))
    #
    df_SBP = pd.DataFrame(SBP_Conf, columns=['y_Actual', 'y_Predicted'])
    df_DBP = pd.DataFrame(DBP_Conf, columns=['y_Actual', 'y_Predicted'])
    #
    confusion_matrix_DBP = pd.crosstab(df_DBP['y_Actual'], df_DBP['y_Predicted'], rownames=['Actual'],
                                       colnames=['Predicted'], normalize='columns')
    confusion_matrix_SBP = pd.crosstab(df_SBP['y_Actual'], df_SBP['y_Predicted'], rownames=['Actual'],
                                       colnames=['Predicted'], normalize='columns')
    #
    plt.figure(figsize=(15, 6))
    plt.subplot(1, 2, 1)
    sns.heatmap(confusion_matrix_SBP, cmap='YlGnBu', annot=True)
    plt.title('Confusion Matrix for SBP Prediction')
    plt.subplot(1, 2, 2)
    sns.heatmap(confusion_matrix_DBP, cmap='YlGnBu', annot=True)
    plt.title('Confusion Matrix for DBP Prediction')
    plt.tight_layout()
